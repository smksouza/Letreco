...
const backspaceAndEnterRow = document.querySelector("#backspaceAndKeyboardRow");
...
for (let rowIndex = 0; rowIndex < rows; rowIndex++) {
  ...
  for (let columnsIndex = 0; columnsIndex < columns; columnsIndex++) {
    ...
    tileColumn.setAttribute("id", "row" + rowIndex + "column" + columnsIndex);
    tileColumn.setAttribute(
      "class",
      rowIndex === 0 ? "tile-column typing" : "tile-column disabled"
    );
    tileRow.append(tileColumn);
    guesses[rowIndex][columnsIndex] = "";
  }
...
const checkGuess = () => {
  ...
  if (guess === letreco) {
    window.alert("tu é demais, simplesmente o detetivão do entreterimento!");
    return;
  } {
    if (currentRow === rows - 1) {
      window.alert("Errrrrrrrrou!");
    } else {
      moveToNextRow();
    }
  }
